...
function toWeekday(date) {
  return moment.weekdays(moment(date, 'YYYY-MM-DD').weekday())
}
...
function fetchSpotify(artistData) {
  ...
  .then(function(result) {
    ...
    artistsRef.child(artistData.id).child('music/spotify').set(url)

    return request.get(urls.spotifyTopTracks(spotifyData.id))
  }).then(function(result) {
    var tracks = result.data.tracks
    if(tracks.length) {
      var preview = tracks[0].preview_url
      artistsRef.child(artistData.id).child('music/preview').set(preview)
    }
  })
}
  ...
  return request.get(urls.bylarmArtist(id)).then(function(result) {
    ...
    data.links.forEach(function(link) {
      if(link.info.type === 'wimp' ||
        ...
        music[link.info.type] = link.link
    })

    var shows = map(data.shows, function(show) {
      var weekday = toWeekday(show.showDateStart)
      var hour = moment(show.showTimeStart, 'HH:mm:ss').format('LT').replace(':00', '')

      lineupRef.child(weekday).child(hour).push(data.main.id)
