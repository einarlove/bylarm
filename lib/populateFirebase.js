...
var spotifyReplacement = {
  'Astrid': '0GvrWrAFJns3Vt1roziDgh'
}

var spotifyBlacklist = ['AV AV AV']
...
var urls = {
  ...
  spotifyArtist: function(name) {
    if(spotifyReplacement[name]) {
      return 'https://api.spotify.com/v1/artists/' + spotifyReplacement[name]
    }

    if(spotifyBlacklist.indexOf(name) > -1) {
      return
    }
...
function fetchSpotify(artistData) {
  ...
  .then(function(result) {
    var spotifyData = result.data.artists ? result.data.artists.items[0] : result.data
